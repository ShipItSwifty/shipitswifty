...
struct ValidateMetadataCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        do {
            ...
            case .json:
                ...
                print(try reporter.encode(ActionResultEnvelope(
                    ...
                    payload: .object([
                        ...
                        "issues": .array(result.warnings.map { .object([
            ...
            case .human:
                outputResult(action: "validate metadata", result: result, format: global.output, colorMode: global.effectiveColorMode)
            }
            ...
            case .json:
                if case .precheckFailed(let violations) = error {
                    ...
                    print(try reporter.encode(ActionResultEnvelope(
                        ...
                        payload: .object([
                            "mode": .string("metadata"),
                            "issues": .array(violations.map { .object([
            ...
            case .json:
                if case .validateArchiveFailed(let issues) = error {
                    ...
                    print(try reporter.encode(ActionResultEnvelope(
                        ...
                        payload: .object([
                            "mode": .string("archive"),
                            "issues": .array(issues.map { .object([
                                "severity": .string("error"),
...
struct ValidateAllCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        if !ymlReport.isValid {
            emitAllFailure(stage: "yml", detail: "Shipfile validation failed", format: global.output, formatter: formatter, report: ymlReport)
            throw ExitCode(2)
        }
        ...
        do {
            ...
            let archiveResult = try await ValidateArchiveAction().run(with: archiveOptions, context: context)
    ...
    private func emitAllFailure(stage: String, detail: String, format: OutputFormat, formatter: HumanFormatter, report: ValidationReport) {
        switch format {
        case .json:
            let reporter = JSONReporter()
            if let encoded = try? reporter.encode(ActionResultEnvelope(
                action: "validate",
                status: "failure",
                payload: .object([
                    "mode": .string("all"),
