...
func outputResult<T: Codable & Sendable>(
    action: String, result: T, format: OutputFormat, colorMode: ConsoleColorMode = .auto
) {
    switch format {
    case .human:
...
func projectInspectionJSON(_ inspection: ProjectInspection) -> JSONValue {
    .object([
        ...
        "xcodeContainers": .array(
            inspection.xcodeContainers.map { container in
                .object([
        ...
        "schemes": .array(
            inspection.schemes.map { scheme in
                .object([
...
func aiSessionPayloadJSON(_ payload: AISessionPayload) -> JSONValue {
    .object([
        ...
        "missing": .array(
            payload.missing.map { missing in
                .object([
                    "keyPath": .string(missing.keyPath),
                    "reason": .string(missing.reason),
                    "envVar": missing.envVar.map(JSONValue.string) ?? .null,
...
private func invalidConfigurationSuggestions(for reason: String) -> [String] {
    ...
    if normalizedReason.contains("requires app.bundle_id")
        || normalizedReason.contains("requires a bundle id")
    {
        return [
    ...
    if normalizedReason.contains("upload requires an ipa path")
        || normalizedReason.contains("testflight requires an ipa path")
    {
        return [
    ...
    if normalizedReason.contains("sign sync requires code_signing.git_url")
        || normalizedReason.contains("sign import requires code_signing.git_url")
    {
        return [
    ...
    if normalizedReason.contains("invalid slack webhook url")
        || normalizedReason.contains("invalid webhook url")
    {
        return [
    ...
    if normalizedReason.contains("git push failed")
        || normalizedReason.contains("git push --tags failed")
    {
        return [
...
private func uploadFailureSuggestions(for reason: String) -> [String] {
    ...
    if normalizedReason.contains("webhook returned non-2xx response")
        || normalizedReason.contains("slack webhook returned non-2xx response")
    {
        return [
