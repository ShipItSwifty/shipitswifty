...
public struct ValidateArchiveAction: Action {
    ...
    public func run(with options: Options, context: ActionContext) async throws -> Result {
        ...
        if let ipa = options.ipaPath {
            ...
            throw ShipItError.invalidConfiguration(
                reason: "validate_archive requires an archive path. Pass --archive-path ./build/MyApp.xcarchive or --ipa ./build/export/MyApp.ipa"
            )
        }
        ...
        if shouldFail {
            throw ShipItError.validateArchiveFailed(issues: issues.map { "[\($0.severity.rawValue.uppercased())] \($0.code): \($0.message)" })
        }
    ...
    private func checkIconAlpha(in appBundle: String, issues: inout [ArchiveIssue]) {
        ...
        if FileManager.default.fileExists(atPath: assetsCar) {
            ...
            issues.append(.init(
                ...
                message: "App icon alpha check skipped: Assets.car requires assetutil to inspect. Ensure the 1024×1024 App Store icon has no transparency.",
                severity: .warning
            ))
            return
        }
    ...
    private func pngHasAlpha(at path: String) -> Bool {
        ...
        guard let data = try? Data(contentsOf: URL(fileURLWithPath: path), options: .mappedIfSafe),
              data.count > 25 else { return false }
    ...
    private func validateUIMainStoryboard(plist: NSDictionary, issues: inout [ArchiveIssue]) {
        ...
        if !hasStoryboard && !hasSceneManifest {
            issues.append(.init(
                ...
                message: "Info.plist has neither UIMainStoryboardFile nor UIApplicationSceneManifest. The app may have no launch interface configured.",
                severity: .warning
            ))
        }
    }
