...
public struct PlayStoreAction: Action {
    ...
    public static let description = "Upload Android release artifact to Google Play and assign to a track"
    ...
    public func run(with options: Options, context: ActionContext) async throws -> Result {
        guard context.platform == .android else {
            throw ShipItError.invalidConfiguration(
                reason: "PlayStoreAction is only supported on Android. Current platform: \(context.platform.rawValue)"
            )
        }

        guard let googlePlay = context.googlePlay else {
            throw ShipItError.invalidConfiguration(
                reason: "Google Play credentials are not configured. Set GOOGLE_PLAY_SERVICE_ACCOUNT_JSON or google_play.service_account_json_path in Shipfile.yml."
            )
        }
        ...
        guard let packageName else {
            throw ShipItError.invalidConfiguration(
                reason: "Play Store upload requires an Android package name. Set android.package_name in Shipfile.yml or pass --package-name."
            )
        }
