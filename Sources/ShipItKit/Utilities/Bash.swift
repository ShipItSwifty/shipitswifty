...
/// A typed wrapper for `/bin/bash` script execution.
public struct Bash: RunnableCommandFamily {
    ...
    /// Typed bash arguments for the selected operation.
    public let arguments: [String]

    /// The shell context used when running this command family.
    public var context: ShellContext { config.context }

    /// Creates a `/bin/bash` command family bound to a shell context.
    ...
    /// Redirects stdout for the built `/bin/bash` command.
    public func settingStdoutDestination(_ destination: OutputDestination) -> Self {
        copy(stdoutDestination: destination)
    }

    /// Redirects stderr for the built `/bin/bash` command.
    public func settingStderrDestination(_ destination: OutputDestination) -> Self {
        copy(stderrDestination: destination)
    }

    /// Uses `/bin/bash -c <script>`.
    public func script(_ value: String) -> Self {
        copy(arguments: ["-c", value])
    }

    /// Builds the raw `/bin/bash` command.
    public func command() -> Command {
        let base = Command("/bin/bash")
            .args(arguments)
