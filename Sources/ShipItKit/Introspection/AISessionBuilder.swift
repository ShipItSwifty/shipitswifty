...
public struct AISessionBuilder: Sendable {
    ...
    private func buildSecretDescriptors(for goal: SuggestionGoal, platform: Platform) -> [SecretDescriptor] {
        switch platform {
        case .android:
            ...
            return [
                SecretDescriptor(
                    keyPath: "android.google_play.service_account_json_path",
                    envVar: "GOOGLE_PLAY_SERVICE_ACCOUNT_JSON",
                    acceptedFormats: ["File path to service account JSON", "Raw JSON string"],
                    exampleSource: "Google Cloud Console → IAM & Admin → Service Accounts → Create Key (JSON)",
                    preferFile: true,
                    description: "Google Play service account JSON for authenticating to the Play Developer API"
                ),
                SecretDescriptor(
                    keyPath: "android.keystore_password",
                    envVar: "ANDROID_KEYSTORE_PASSWORD",
    ...
    private func buildReadiness(
        ...
        case .android:
            ...
            if (goal == .beta || goal == .release) {
                ...
                unblockSteps.append("Set ANDROID_KEYSTORE_PATH, ANDROID_KEYSTORE_PASSWORD, ANDROID_KEY_ALIAS, ANDROID_KEY_PASSWORD")
            }
        }
    ...
    private func buildNextAction(
        ...
        if !hasExistingShipfile {
            return NextAction(
                ...
                command: "shipit suggest-config --goal \(goal.rawValue)\(platformFlag) --output json",
    ...
    private func buildAgentPrompt(goal: SuggestionGoal, readiness: AIReadiness, platform: Platform) -> String {
        ...
        case .android:
            lines = [
                ...
                "  shipit suggest-config --goal \(goal.rawValue) --platform android # generate starter Shipfile",
        ...
        case .android:
            if goal == .beta {
                lines += [
                    ...
                    "Android signing requires ANDROID_KEYSTORE_PATH, ANDROID_KEYSTORE_PASSWORD, ANDROID_KEY_ALIAS, ANDROID_KEY_PASSWORD.",
    ...
    private func buildNextQuestion(
        ...
        case .android:
            ...
            if (goal == .beta || goal == .release) && !readiness.missingSecrets.isEmpty {
                return "Do you have your Google Play service account JSON ready? You'll need: GOOGLE_PLAY_SERVICE_ACCOUNT_JSON, ANDROID_KEYSTORE_PATH, ANDROID_KEYSTORE_PASSWORD, ANDROID_KEY_ALIAS, ANDROID_KEY_PASSWORD."
            }
            ...
            if goal == .beta {
                return "The Android beta workflow will build a release AAB and upload to the internal track on Google Play. Do you want to target a different track (alpha, beta, production)?"
            }
