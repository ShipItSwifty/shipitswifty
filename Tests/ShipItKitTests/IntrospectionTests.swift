...
struct IntrospectionTests {
    ...
    func projectInspectorDetectsWorkspaceAndSchemes() async throws {
        ...
        try FileManager.default.createDirectory(at: tempDirectory.appendingPathComponent("Example.xcworkspace"), withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: tempDirectory.appendingPathComponent("Example.xcodeproj"), withIntermediateDirectories: true)

        let executor = MockExecutor { command, _ in
            if command.arguments.contains("-list") {
                return ShellOutput(
                    stdout: "Information about workspace \"Example\":\n    Schemes:\n        Example\n        ExampleTests\n",
                    stderr: "",
                    exitCode: 0
                )
            }
            ...
            if command.arguments.contains("-showBuildSettings") {
                return ShellOutput(
                    stdout: "    PRODUCT_BUNDLE_IDENTIFIER = com.example.app\n    DEVELOPMENT_TEAM = TEAM12345\n",
                    stderr: "",
                    exitCode: 0
                )
            }
    ...
    func shipfileSuggesterGeneratesBetaWorkflow() {
        let inspection = ProjectInspection(
            ...
            schemes: [.init(name: "App", containerPath: "App.xcworkspace", bundleID: "com.example.app", teamID: "TEAM12345", likelyRunnable: true)],
