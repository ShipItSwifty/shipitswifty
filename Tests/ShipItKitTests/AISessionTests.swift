...
struct AISessionTests {
    ...
    func assumedDefaultsPresent() {
        let payload = AISessionBuilder().build(goal: .beta, inspection: fullInspection(), hasExistingShipfile: false)
    ...
    func emptyProjectProducesBlockers() {
        let payload = AISessionBuilder().build(goal: .beta, inspection: emptyInspection(), hasExistingShipfile: false)

        #expect(!payload.readiness.isReady)
        #expect(payload.readiness.blockers.contains { $0.contains("workspace") || $0.contains("project") })
    ...
    func localGoalIncludesNoSecrets() {
        let payload = AISessionBuilder().build(goal: .local, inspection: fullInspection(), hasExistingShipfile: false)
    ...
    func ambiguityTakesPriorityOverShipfileCreation() {
        let inspection = inspectionWithMultipleRunnableSchemes()
        let payload = AISessionBuilder().build(goal: .local, inspection: inspection, hasExistingShipfile: false)
    ...
    func readyWithNoShipfileProducesCreateShipfileAction() {
        let payload = AISessionBuilder().build(goal: .local, inspection: localReadyInspection(), hasExistingShipfile: false)
        #expect(payload.nextAction.action == "create_shipfile")
        #expect(payload.nextAction.command?.contains("suggest-config") == true)
    }

    @Test("Ready, existing shipfile, no missing => run_workflow with command")
    func readyWithShipfileProducesRunWorkflowAction() {
        let payload = AISessionBuilder().build(goal: .local, inspection: localReadyInspection(), hasExistingShipfile: true)
    ...
    func agentPromptContainsGoalAndCommands() {
        ...
        #expect(payload.agentPrompt.contains("suggest-config"))
    }
    ...
    func ambiguitiesProduceNextQuestion() {
        ...
        #expect(payload.nextQuestion?.contains("App") == true || payload.nextQuestion?.contains("scheme") == true)
    }
    ...
    private func fullInspection() -> ProjectInspection {
        ProjectInspection(
            ...
            schemes: [
                ...
                .init(name: "AppTests", containerPath: "App.xcworkspace", bundleID: nil, teamID: nil, likelyRunnable: false),
            ],
    ...
    private func inspectionWithMultipleRunnableSchemes() -> ProjectInspection {
        ProjectInspection(
            ...
            schemes: [
                .init(name: "App", containerPath: "App.xcworkspace", bundleID: "com.example.app", teamID: "TEAM12345", likelyRunnable: true),
                .init(name: "AppExtension", containerPath: "App.xcworkspace", bundleID: "com.example.app.ext", teamID: "TEAM12345", likelyRunnable: true),
            ],
            suggestedAppConfig: .init(workspace: "App.xcworkspace", scheme: "App", bundleID: "com.example.app", teamID: "TEAM12345"),
            existingShipfiles: [],
            fastlaneFiles: [],
