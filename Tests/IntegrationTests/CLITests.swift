...
struct CLIIntegrationTests {
    ...
    func validateYmlFailsForMalformedYAML() async throws {
        ...
        #expect(result.exitCode != 0, "Expected non-zero exit for malformed YAML, got: \(result.exitCode)")
    }
    ...
    func aiSessionProducesValidJSON() async throws {
        ...
        #expect(json?["action"] != nil || json?["version"] != nil, "Expected envelope or versioned JSON in ai-session response")
    }

    // MARK: - Init

    @Test("init --non-interactive --output json exits 0")
    func initNonInteractiveExitsZero() async throws {
        let tmp = FileManager.default.temporaryDirectory
        ...
        let result = try await CLI.run(
            "init", "--goal", "beta", "--non-interactive", "--output", "json",
            workingDirectory: tmp
        )
        // May exit non-zero if no Xcode project found, but must not crash
        #expect(result.exitCode == 0 || result.exitCode == 1)
        #expect(result.stderr.isEmpty || !result.stderr.contains("fatal"))
    }
}
