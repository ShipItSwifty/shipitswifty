...
struct AndroidCLITests {
    ...
    @Test("all command families construct expected prefixes", arguments: [
        ...
        (AndroidCLI().screenCapture(output: "screen.png", annotate: true).command().arguments, ["screen", "capture", "--annotate", "--output=screen.png"]),
    ...
    func layoutParser() throws {
        let elements = try AndroidCLIOutputParser.layoutElements(from: """
