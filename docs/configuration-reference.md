...
| `rerun_failed_tests` | object | — | Selectively rerun only the failed tests when the runner supports it. Supports `enabled` and `max_attempts` (default `2`). Supported for Android unit tests (`kind: unit`; failed tests come from JUnit XML or the Gradle log and are re-run with `--tests`) and for iOS with a single destination plus `result_bundle_path` (failed tests come from the xcresult and are re-run with `-only-testing`). The step fails if any test fails again; if every failure passes on rerun it succeeds and reports the tests as flaky. |
