...
| **AI session** | `shipit ai session --goal <local\|beta\|release>` returns a **versioned, stable JSON contract** (v1) containing inferred config with confidence + provenance, secret descriptors, ambiguity flags, readiness diagnosis, next deterministic action, agent system prompt, and the single best follow-up question to ask the user. The prompt preserves the generated infrastructure-retry default and tells agents how to use a detected iOS test plan or ask about multiple plans. Also available under `shipit ai`: `shipit ai prompt` (just the `agentPrompt` field) and `shipit ai instructions` (ShipItSwifty's own operating guidance for agents, no project inspection required). |
| **AI skills (playbooks)** | `shipit ai skills list` / `shipit ai skills show <id>` print a small, built-in catalog of task-oriented agent playbooks (`react-native-setup`, `firebase-distribution`, `migrate-from-fastlane`, `kmp-dual-platform`, `custom-actions`) — short, curated "how to do X" guides distinct from the full reference docs, defined in `AISkillsCatalog.swift`. Fixed and in-binary; not an installable package registry. |
...
| **Version Source — xcconfig** | Reads and writes named keys directly in an `.xcconfig` file via line-based replacement. Configure with `versioning.spec_path` (the `.xcconfig` path), `versioning.marketing_key`, and `versioning.build_key`. Use when the version is a single source of truth in an `.xcconfig` referenced via `$(VAR)` from the `.xcodeproj` (e.g. `Config/Version.xcconfig`); only the named keys are rewritten, so the `$(VAR)` indirection survives. |
...
| **Version Source — pubspec** | Reads and writes the `version: X.Y.Z+B` key in `pubspec.yaml` via line-based replacement (marketing version before `+`, build number after). Default versioning source for Flutter projects — `flutter build` stamps the native projects from pubspec, so writing `xcodeproj`/`gradle` directly would be overwritten. Configure the file location with `versioning.spec_path` (defaults to `pubspec.yaml`). Pure file I/O (Linux-compatible). |
| **Version Target Override** | The `version` action accepts a `target` option (`source_of_truth`, `project_spec`, `xcodeproj`, `kmp`, `gradle`, `xcconfig`, or `pubspec`) to override the configured versioning source per-step, enabling workflows that write to the spec file even when the default source is `xcodeproj`. |
...
|---|---|---|
| **AndroidCLI integration** | `shipit android-cli <command>` and `android-*` workflow actions | Opt-in typed support for all AndroidCLI 1.0 command families: create/describe, deploy/run, emulator and SDK management, layout/screen inspection, docs, skills, and Android Studio integration. Gradle remains the build/test/archive/lint engine; mutating workflow operations require `allow_mutation: true`, and mutating `shipit android-cli` invocations require `--allow-mutation`. Both `shipit android-cli` and the `android-*` workflow actions enforce the AndroidCLI 1.0+ minimum version check and pick up `android.cli.executable_path` / `android.cli.sdk_path` from Shipfile.yml when present. |
